...
function createImageCard(item) {
    ...
    card.innerHTML = `
        <div class="flex items-center p-3 gap-4">
            <div class="w-16 h-16 bg-gray-50 flex-shrink-0 rounded-xl overflow-hidden border border-gray-50 flex items-center justify-center">
                <img id="result-${item.id}" class="max-w-full max-h-full object-contain" data-zoomable />
            </div>
            <div class="flex-1 min-w-0">
                <h4 class="font-bold text-xs text-gray-900 truncate mb-1">${item.name}</h4>
                <div class="text-[10px] text-gray-400 font-medium" id="status-${item.id}">${i18n.t('status.pending')}</div>
            </div>
            <div class="flex-shrink-0">
                <button id="download-${item.id}" class="px-4 py-2 bg-primary/10 text-primary hover:bg-primary hover:text-white rounded-lg text-xs font-bold transition-all hidden">${i18n.t('btn.download')}</button>
            </div>
        </div>
    `;
